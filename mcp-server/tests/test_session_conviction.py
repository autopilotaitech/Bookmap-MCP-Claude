...
# ─── V10 (fixed): bias_score reads flow.biasTrajectory from FlowRegime ─────

def test_bias_score_no_trajectory_field_is_flat_no_nudge():
    """Older bridge payloads without biasTrajectory must work — default FLAT."""
    out = d._source_bias_score({"flow": {"biasScore": 0.5}})
    assert out["raw"]["trajectory"] == "FLAT"
    assert out["score"] == pytest.approx(0.5, abs=1e-9)


def test_bias_score_rising_strong_from_flow_adds_nudge():
    out = d._source_bias_score({"flow":
        {"biasScore": 0.5, "biasTrajectory": "RISING_STRONG"}})
    assert out["raw"]["trajectory"] == "RISING_STRONG"
...
def test_bias_score_falling_strong_with_positive_score_diverges():
    """Positive bias + FALLING_STRONG = exhaustion. Strong divergence → rel 0.5."""
    out = d._source_bias_score({"flow":
        {"biasScore": 0.3, "biasTrajectory": "FALLING_STRONG"}})
    assert out["raw"]["trajectory"] == "FALLING_STRONG"
    assert "DIVERGED" in out["reason"]
    assert out["reliability"] == pytest.approx(0.5, abs=1e-9)
...
def test_bias_score_mild_divergence_cuts_reliability_partially():
    out = d._source_bias_score({"flow":
        {"biasScore": 0.5, "biasTrajectory": "FALLING"}})
    assert out["raw"]["trajectory"] == "FALLING"
    assert out["reliability"] == pytest.approx(0.75, abs=1e-9)
...
def test_bias_score_confirming_trajectory_keeps_full_reliability():
    ...
    assert out["raw"]["trajectory"] == "RISING"
    assert "DIVERGED" not in out["reason"]
    assert out["reliability"] == 1.0


def test_bias_score_warmup_trajectory_is_neutral():
    """FlowRegime emits WARMUP before baselines build. No nudge, no divergence."""
    out = d._source_bias_score({"flow":
        {"biasScore": 0.4, "biasTrajectory": "WARMUP"}})
    assert out["score"] == pytest.approx(0.4, abs=1e-9)
    assert out["reliability"] == 1.0
    assert "DIVERGED" not in out["reason"]


def test_bias_score_unknown_trajectory_is_neutral():
    """Any non-recognized trajectory string defaults to no nudge / no divergence."""
    out = d._source_bias_score({"flow":
        {"biasScore": 0.4, "biasTrajectory": "ZONKED"}})
    assert out["score"] == pytest.approx(0.4, abs=1e-9)
    assert out["reliability"] == 1.0


def test_bias_score_missing_biasscore_returns_zero_reliability():
    """Sanity: missing biasScore field still produces zero-reliability output."""
    out = d._source_bias_score({"flow": {}})
    assert out["score"] == 0.0
